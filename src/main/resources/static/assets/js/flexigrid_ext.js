    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                if (p.dataType == 'xml')
                    ...
                    p.page = data[p.config.queryPage];
                ...
                if (p.dataType == 'json') {
                    p.jsonData || (p.jsonData = data[p.config.list]);
                    if (typeof(data[p.config.list]) == 'undefined') {
                        $(tbody).append('<tr><td colspan="100" class="no-data"><i class="fa fa-exclamation-circle"></i>' + p.nomsg + '</td></tr>');
                    } else {
                        $.each(data[p.config.list], function (i, row) {
                                ...
                                //add cell
                                $(p.colModel).each(function (e) {
                                    var names = this.name.split(".");  //处理下级对象
                                    ...
                                    if (item === undefined && !this.formatter) {
                                        item = "";
                                    } else if (this.formatter && this.formatter.constructor === Function) {
                                        //formatter处理
                                        item = this.formatter.call(this, item, p, row);
                                    }

                                    td.innerHTML = item;
                                    $(tr).append(td);
                                    if (e === p.colModel.length - 1) {
            ...
            populate: function () { //get latest data
                ...
                var param = [
                    {name: p.config.curpage, value: p.newp}
                    , {name: p.config.rp, value: p.rp}
                    , {name: p.config.sortname, value: p.sortname}
                    , {name: p.config.sortorder, value: p.sortorder}
                    , {name: p.qtype, value: p.query}
                    //, {name: p.config.qtype, value: p.qtype}
                ];


                if (p.params) {
                    if (p.params.constructor === Array) {
                        for (var pi = 0; pi < p.params.length; pi++) param[param.length] = p.params[pi];
                    } else if (p.params.constructor === Object) {
                        for (var x in p.params) {
                            var obj = {};
                            obj.name = x;
                            obj.value = p.params[x];
            ...
            doSearch: function () {
                ...
                p.qtype = $('select[name=qtype]', g.sDiv).attr("data-name");
                p.newp = 1;
            ...
            for (i = 0; i < p.buttons.length; i++) {
                ...
                if (!btn.separator) {
                    ...
                    if (btn.bclass)
                        ...
                        ;
                    if(btn.permiss!=null&&btn.permiss!=''){
                        $('div', btnDiv).attr('permiss',btn.permiss).hide()
                    }
                    ...
                    if (btn.onpress) {
                        $(btnDiv).click(
                            function () {
                                this.onpress(this.name, g.bDiv, p.jsonData);
                            }
                        );
                    } else if (btn.onMultiple) { //用于选择多行记录做批量改变动作
                        btnDiv.onMultiple = btn.onMultiple;
                        $(btnDiv).click(
                            function () {
                                var onData = new Array();   //被选中行对应的数据
                                //被选中的行
                                $(g.bDiv).find("tr.trSelected").each(function (i) {
                                    onData[i] = p.jsonData[this.rowIndex];
                                });
                                this.onMultiple(this.name, onData.length === 0 ? false : true, onData);
                            }
                        );
                    }
                    $(tDiv2).append(btnDiv);
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'><select name='qtype' data-name='' class='select'>" + sopt + "</select> <input type='text' size='30' name='q' class='qsbox'  placeholder='搜索相关数据...'/> <input type='button' class='btn' value='" + p.findtext + "' /></div>");

                var first_opt = $("select[name='qtype']", g.sDiv).find("option").eq(0); //第一个option

                $("select[name='qtype']", g.sDiv).attr("data-name", first_opt.val()).change(function () {
